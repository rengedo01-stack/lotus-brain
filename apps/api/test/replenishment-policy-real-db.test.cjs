...
const databaseUrl = readDisposableDatabaseUrl("REPLENISHMENT_POLICY_DATABASE_URL");
const allowedDatabaseName = "lotus_brain_pr006c6_replenishment_policy_test";
...
if (databaseUrl === undefined) {
  ...
  if (databaseName !== allowedDatabaseName) {
    ...
    test("replenishment policy keeps inventory-unit targets, permissions, and optimistic updates exact", async () => {
      ...
      const fixture = `c6-${randomUUID().replace(/-/g, "").slice(0, 18)}`;
      let app;
      ...
      try {
        const unit = await prisma.unit.create({ data: { code: `${fixture}-EA`, name: "PR-006C6 each", symbol: "ea", dimension: "COUNT", status: "ACTIVE" } });
        const product = await prisma.product.create({ data: { code: `${fixture}-ACTIVE`, name: "PR-006C6 active", baseUnitId: unit.id, inventoryUnitId: unit.id } });
        const concurrentProduct = await prisma.product.create({ data: { code: `${fixture}-CONCURRENT`, name: "PR-006C6 concurrent", baseUnitId: unit.id, inventoryUnitId: unit.id } });
        const inactiveProduct = await prisma.product.create({ data: { code: `${fixture}-INACTIVE`, name: "PR-006C6 inactive", baseUnitId: unit.id, inventoryUnitId: unit.id, status: "INACTIVE" } });
        const deletedProduct = await prisma.product.create({ data: { code: `${fixture}-DELETED`, name: "PR-006C6 deleted", baseUnitId: unit.id, inventoryUnitId: unit.id, status: "INACTIVE", deletedAt: new Date("2026-09-09T00:00:00.000Z") } });
        ...
        assert.deepEqual(
          swaggerPath.post.requestBody.content["application/json"].schema.required,
          ["reorderPointQuantity", "targetStockQuantity"],
        );
        assert.deepEqual(
          swaggerPath.patch.requestBody.content["application/json"].schema.required,
          ["reorderPointQuantity", "targetStockQuantity", "expectedVersion"],
        );
        let requestNumber = 0;
        ...
        assert.equal((await request("POST", path(product.id), readerSession, { reorderPointQuantity: "1", targetStockQuantity: null })).status, 403);
        ...
        assert.equal((await request("POST", path(product.id), adminSession, { reorderPointQuantity: "-1", targetStockQuantity: null })).status, 400);
        assert.equal((await request("POST", path(product.id), adminSession, { reorderPointQuantity: "01", targetStockQuantity: null })).status, 400);
        assert.equal((await request("POST", path(product.id), adminSession, { reorderPointQuantity: "1e3", targetStockQuantity: null })).status, 400);
        assert.equal((await request("POST", path(product.id), adminSession, { reorderPointQuantity: "1" })).status, 400);
        assert.equal((await request("POST", path(product.id), adminSession, { reorderPointQuantity: "1", targetStockQuantity: "-1" })).status, 400);
        assert.equal((await request("POST", path(product.id), adminSession, { reorderPointQuantity: "5", targetStockQuantity: "4.999999999" })).status, 422);
        const created = await request("POST", path(product.id), adminSession, { reorderPointQuantity: "0", targetStockQuantity: null });
        assert.equal(created.status, 201);
        const createdBody = await created.json();
        assert.deepEqual(Object.keys(createdBody).sort(), ["createdAt", "id", "productId", "reorderPointQuantity", "targetStockQuantity", "updatedAt", "version"]);
        assert.equal(createdBody.productId, product.id);
        assert.equal(createdBody.reorderPointQuantity, "0");
        assert.equal(createdBody.targetStockQuantity, null);
        assert.equal(createdBody.version, 1);
        assert.equal((await request("POST", path(product.id), adminSession, { reorderPointQuantity: "1", targetStockQuantity: null })).status, 409);

        const updated = await request("PATCH", path(product.id), adminSession, { reorderPointQuantity: "999999999999999.123456789", targetStockQuantity: "999999999999999.123456789", expectedVersion: 1 });
        assert.equal(updated.status, 200);
        const updatedBody = await updated.json();
        assert.equal(updatedBody.reorderPointQuantity, "999999999999999.123456789");
        assert.equal(updatedBody.targetStockQuantity, "999999999999999.123456789");
        assert.equal(updatedBody.version, 2);
        assert.equal((await request("PATCH", path(product.id), adminSession, { reorderPointQuantity: "3", targetStockQuantity: "3", expectedVersion: 1 })).status, 409);
        assert.equal((await request("PATCH", path(product.id), adminSession, { reorderPointQuantity: "3", targetStockQuantity: "3", expectedVersion: 0 })).status, 400);

        const targetProduct = await prisma.product.create({ data: { code: `${fixture}-TARGET`, name: "PR-006C6 target", baseUnitId: unit.id, inventoryUnitId: unit.id } });
        const targetCreated = await request("POST", path(targetProduct.id), adminSession, { reorderPointQuantity: "5", targetStockQuantity: "15" });
        assert.equal(targetCreated.status, 201);
        assert.equal((await targetCreated.json()).targetStockQuantity, "15");
        const targetUpdated = await request("PATCH", path(targetProduct.id), adminSession, { reorderPointQuantity: "5", targetStockQuantity: "5", expectedVersion: 1 });
        assert.equal(targetUpdated.status, 200);
        assert.equal((await targetUpdated.json()).targetStockQuantity, "5");
        const targetCleared = await request("PATCH", path(targetProduct.id), adminSession, { reorderPointQuantity: "5", targetStockQuantity: null, expectedVersion: 2 });
        assert.equal(targetCleared.status, 200);
        assert.equal((await targetCleared.json()).targetStockQuantity, null);

        assert.equal((await request("POST", path(inactiveProduct.id), adminSession, { reorderPointQuantity: "1", targetStockQuantity: null })).status, 409);
        assert.equal((await request("POST", path(deletedProduct.id), adminSession, { reorderPointQuantity: "1", targetStockQuantity: null })).status, 409);
        assert.equal((await request("POST", path("not-a-product"), adminSession, { reorderPointQuantity: "1", targetStockQuantity: null })).status, 404);

        const [createA, createB] = await Promise.all([
          request("POST", path(concurrentProduct.id), adminSession, { reorderPointQuantity: "1", targetStockQuantity: "1" }),
          request("POST", path(concurrentProduct.id), adminSession, { reorderPointQuantity: "2", targetStockQuantity: "2" }),
        ]);
        ...
        const [updateA, updateB] = await Promise.all([
          request("PATCH", path(concurrentProduct.id), adminSession, { reorderPointQuantity: "3", targetStockQuantity: "3", expectedVersion: concurrentPolicy.version }),
          request("PATCH", path(concurrentProduct.id), adminSession, { reorderPointQuantity: "4", targetStockQuantity: "4", expectedVersion: concurrentPolicy.version }),
        ]);
        ...
        try {
          ...
          const waitingCreate = request("POST", path(lifecycleRaceProduct.id), adminSession, { reorderPointQuantity: "1", targetStockQuantity: null });
        ...
        await assert.rejects(
          () => prisma.$executeRawUnsafe(`INSERT INTO "ReplenishmentPolicy" ("id", "productId", "reorderPointQuantity", "targetStockQuantity", "version", "createdAt", "updatedAt") VALUES ('${fixture}-target-below', '${deletedProduct.id}', 5, 4.999999999, 1, NOW(), NOW())`),
          /target_at_least_reorder/i,
        );
